...
leadSchema.methods.getSpaceBreakdown = function () {
  ...
  if (spaceRequirements.workstations.persons > 0) {
    breakdown.push(
      `${spaceRequirements.workstations.persons} Workstations (${spaceRequirements.workstations.type})`,
    );
  }
  ...
  if (spaceRequirements.pantry.count > 0) {
    breakdown.push(
      `${spaceRequirements.pantry.count} Pantry(s) (${spaceRequirements.pantry.type})`,
    );
  }

  if (spaceRequirements.conferenceRoom.count > 0) {
    breakdown.push(
      `${spaceRequirements.conferenceRoom.count} Conference Room(s) (${spaceRequirements.conferenceRoom.type})`,
    );
  }
